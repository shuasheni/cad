...
from joint_predict.joint.joint_set import JointSet
...
class JointPredictionSet(JointSet):
    def __init__(
        ...
        super().__init__(joint_file, load_bodies=load_bodies, new_joint=new_joint, g1_id=g1_id, g2_id=g2_id)
        # Run inference and store the predictions in a cacheable data structure
        self.prediction_data, self.av = self.get_prediction_data(joint_file, g1, g2, joint_graph, model, prediction_limit, new_joint=new_joint, g1_id=g1_id, g2_id=g2_id)
        ...
        # 应用body two变换，使我们处于正确的坐标系中
        # 我们进行第一次联合变换，假设它们是恒定的
    ...
    def get_network_predictions(g1, g2, joint_graph, model):
        ...
        # print("Get the network predictions: g1:")
        # print(g1)
        # print("g2:")
        # print(g2)
        fake_batch = (
    ...
    def get_prediction_data(self, joint_file, g1, g2, joint_graph, model, prediction_limit, new_joint=False, g1_id=None, g2_id=None):
        """
        生成预测数据结构
        返回一个可以序列化为json进行缓存的字典
        """
        # g1 and g2 are the B-Rep bodies represented as graphs with learning features
        # label_matrix具有用户选择的关节
        ...
        joint_data["body_one_edge_count"] = g1_graph["properties"]["edge_count"]
        joint_data["body_two_face_count"] = g2_graph["properties"]["face_count"]
        joint_data["body_two_edge_count"] = g2_graph["properties"]["edge_count"]
        ...
        if len(joint_data["predictions"]) == 0:
            ...
            return joint_data, False
        return joint_data, True
    ...
    def is_joint_body_rotationally_symmetric(self, body=1, prediction_index=0, joint_axis_direction=None):
        ...
        # print(f"joint_axis_direction:{joint_axis_direction}")
        # print(f"pairs:{pairs}")
