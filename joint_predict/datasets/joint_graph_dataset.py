...
from joint_predict.datasets.joint_base_dataset import JointBaseDataset
...
class JointGraphDataset(JointBaseDataset):
    ...
    hole_type_map = {
        "None": 0,
        "RoundHoleWithBlindBottom": 1,
        "RoundBlindHoleWithConicalBottom": 2,
        "RoundBlindHoleWithSphericalBottom": 3,
        "RoundBlindHoleWithTaperedBottom": 4,
        "RoundHoleWithThroughBottom": 5,
        "TaperedRoundHoleWithBlindBottom": 6,
        "TaperedRoundHoleWithThroughBottom": 7,
        "CounterboreBlindHole": 8,
        "CounterboreBlindHoleWithConicalBottom": 9,
        "CounterboreBlindHoleWithSphericalBottom": 10,
        "CounterboreBlindHoleWithTaperedBottom": 11,
        "CounterboreThroughHole": 12,
        "TaperedCounterboreBlindHole": 13,
        "TaperedCounterboreHoleWithThroughBottom": 14,
        "CountersunkHoleWithBlindBottom": 15,
        "CountersunkBlindHoleWithConicalBottom": 16,
        "CountersunkBlindHoleWithSphericalBottom": 17,
        "CountersunkBlindHoleWithTaperedBottom": 18,
        "CountersunkHoleWithThroughBottom": 19,
        "CountersunkCounterboreBlindHole": 20,
        "CountersunkCounterboreBlindHoleWithConicalBottom": 21,
        "CountersunkCounterboreBlindHoleWithSphericalBottom": 22,
        "CountersunkCounterboreBlindHoleWithTaperedBottom": 23,
        "CountersunkCounterboreThroughHole": 24
    }
    ...
    def scale_geometry(
        ...
        hole_length_features1=None,
        hole_length_features2=None,
    ):
        ...
        if hole_length_features1 is not None:
            hole_length_features1 *= scale
        if hole_length_features2 is not None:
            hole_length_features2 *= scale
        return True
    ...
    def load_graph_body(self, body, holes):
        ...
        for index, node in enumerate(g_json["nodes"]):
            ...
            if "is_degenerate" in node and node["is_degenerate"]:
                ...
                node["is_hole_face"] = torch.tensor(0, dtype=torch.long)
                # 对于边实体，是否做为一个孔边
                node["is_hole_edge"] = torch.tensor(0, dtype=torch.long)
                # 对于孔面或孔边的孔类型
                node["hole_type"],_ = self.get_hole_type("None")
                # 对于孔面或孔边的孔长度
                node["hole_length"] = torch.tensor(0, dtype=torch.long)
            else:
                ...
                is_hole_face = torch.tensor(0, dtype=torch.long)
                is_hole_edge = torch.tensor(0, dtype=torch.long)
                hole_type = torch.tensor(0, dtype=torch.long)
                hole_length = torch.tensor(0, dtype=torch.long)

                if "surface_type" in node:
                    is_hole_face, hole_type, hole_length = self.get_node_is_hole_face(body, node, holes)
                elif "curve_type" in node:
                    is_hole_face, hole_type, hole_length = self.get_node_is_hole_edge(body, node, holes)

                # 对于面实体，是否做为一个孔面
                node["is_hole_face"] = is_hole_face
                # 对于边实体，是否做为一个孔边
                node["is_hole_edge"] = is_hole_edge
                # 对于孔面或孔边的孔类型
                node["hole_type"] = hole_type
                # 对于孔面或孔边的孔长度
                node["hole_length"] = hole_length
    ...
    def reshape_graph_features(self, g):
        ...
        # g.hole_type = torch.cat(g.hole_type).reshape(
        #     (-1, len(self.hole_type_map)))
        return g
    ...
    def get_hole_type(self, type):
        hole_type = self.hole_type_map[type]
        hole_type_tensor = torch.tensor(hole_type, dtype=torch.long)
        num_hole_type_classes = len(self.hole_type_map)
        hole_type_one_hot = F.one_hot(hole_type_tensor, num_classes=num_hole_type_classes)
        return hole_type_one_hot, hole_type
    ...
    def get_ambiguous_labels(self, g, joint_entities):
        ...
        for index, entity in enumerate(joint_entities):
            ...
            # entity_hole = int(torch.argmax(g.hole[entity]))
            ...
            for i in range(g.num_nodes):
                ...
                # label_hole = int(torch.argmax(g.hole[i]))
    ...
    def get_node_is_hole_face(self, body, id, holes):
        for hid, hole in enumerate(holes):
            if body != hole["body"]:
                continue
            for fid, face in enumerate(hole["faces"]):
                if id == face["id"]:
                    return torch.tensor(1, dtype=torch.long), self.get_hole_type(hole["type"]), torch.tensor(hole["length"], dtype=torch.long)

        hole_type_hot,_ = self.get_hole_type("None")
        return torch.tensor(0, dtype=torch.long), hole_type_hot, torch.tensor(0, dtype=torch.long)

    def get_node_is_hole_edge(self, body, id, holes):
        for hid, hole in enumerate(holes):
            if body != hole["body"]:
                continue
            for eid, edge in enumerate(hole["edges"]):
                if id == edge["id"]:
                    return torch.tensor(1, dtype=torch.long), self.get_hole_type(hole["type"]), torch.tensor(hole["length"], dtype=torch.long)
